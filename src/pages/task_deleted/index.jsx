...
import UserTaskDeletedList from "./UserTaskDeletedList"
...
const TaskDeleted = () => {
  ...
  const restoreAllTask = async () => {
    try {
      const res = await api.post('/task_deleted/restore_all', null, {
        headers: {
  ...
  const restoreTaskById = async (taskId) => {
    try {
      const res = await api.post(`/task_deleted/${taskId}/restore`, null, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
  ...
  return (
    <>
      ...
      <header>
        <Navbar token={token} handleOpenSidebar={handleOpenSidebar} disable={disable} handleShowWarningMessage={handleShowWarningMessage} restoreAllTask={restoreAllTask}/>
      </header>
        ...
        <div className="container mx-auto flex">
          <aside className="w-1/4 lg:block hidden bg-body border-navbar border-r h-screen py-3 px-4">
            <div className="flex flex-col justify-between h-[92%] mt-16 py-5">
              ...
              <CommingSoon />
              <Logout logout={logout} showForm={showForm} setShowForm={setShowForm} showUploadImageForm={showUploadImageForm} hideUploadForm={hideUploadForm} uploadProfileImage={uploadProfileImage} profileUrl={profileUrl} />
            </div>
          </aside>
          <main className="w-full h-screen lg:flex-1 overflow-y-auto">
            <div className={`mt-16 py-3 px-4 h-[90%] ${tasksDeleted.length === 0 && 'py-5'}`}>
              {isLoading ? (
                <div  className="h-full flex justify-center items-center">
                  <img src={Rolling} alt="loadin-svg" className="w-9" />
                </div>
              ) : (
                tasksDeleted.length === 0 ? (
                  ...
                  <section className="deleted-list">
                    <UserTaskDeletedList
                      ...
                      restoreTaskById={restoreTaskById}
                    />
                  </section>
                )
              )}
